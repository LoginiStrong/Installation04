...
void Scanner::advance()
{
	...
	while (start)
	{
		...
		//if the character is a (-) it checks to see if the next character is a decimal or a number to get the float value, if not it's just a minus
		else if (buffer[0] == '-')
		{
			addToBuffer(buffer, filestream);
			if (isNum(buffer[1]))
			{
				int decimalCount = 0;
				addToBuffer(buffer, filestream);
				while ((isNum(buffer[buffer.size() - 1]) || isDecimal(buffer[buffer.size() - 1])) && decimalCount < 2)
				{
					addToBuffer(buffer, filestream);
		...
		else if (buffer[0] == '.') //for adding a float with a leading decimal without a minus
		{
			addToBuffer(buffer, filestream);
			if (isNum(buffer[1]))
			{
		...
		{
			int decimalCount = 0;//counts if it runs into a decimal so that if it see 2 it cuts the float off
			addToBuffer(buffer, filestream);
			while ((isNum(buffer[buffer.size() - 1]) || isDecimal(buffer[buffer.size() - 1])) && decimalCount < 2)
			{
				addToBuffer(buffer, filestream);
